...
def plot_system_2d(charges, target_charge=None, net_force=None, selected_idx=None):
    ...
    if target_charge and net_force is not None:
        ...
        if force_mag > 1e-15:
            ...
            x1, y1 = (x0 + nx, y0 + ny)
            fig.add_annotation(x=x1, y=y1, ax=x0, ay=y0, xref='x', yref='y', axref='x', ayref='y', showarrow=True, arrowhead=3, arrowsize=1.8, arrowwidth=3, arrowcolor='#16a34a', font=dict(color='#16a34a', size=12))
            fig.add_annotation(x=x1 + 0.08, y=y1 + 0.08, text=f'F = {force_mag:.3e} N', showarrow=False, font=dict(color='#16a34a', size=11))
    if x_pos and y_pos:
...
def plot_system_2d_mpl(charges, target_charge=None, net_force=None, filename=None):
    ...
    for i, c in enumerate(charges):
        ax.annotate(f'q{i + 1}\n({c.q:.1e} C)', (c.x, c.y), textcoords='offset points', xytext=(0, 10), ha='center')
    if target_charge and net_force is not None:
        ax.quiver(target_charge.x, target_charge.y, net_force[0], net_force[1], color='green', scale_units='xy', scale=None, label='Fuerza Neta')
        ax.legend()
    ax.set_xlabel('X (m)')
    ax.set_ylabel('Y (m)')
