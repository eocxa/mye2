...
def get_float_input(prompt):
    """Solicita un valor flotante al usuario con validación."""
    while True:
        try:
...
def get_positive_int_input(prompt):
    """Solicita un entero positivo al usuario con validación."""
    while True:
        try:
...
def is_duplicate_position(charges, x, y):
    """Verifica si ya existe una carga en la posición (x, y)."""
    return any(c.x == x and c.y == y for c in charges)


def collect_charges(mode):
    """Solicita los datos de las cargas al usuario."""
...
def select_target_charge(charges):
    """Muestra las cargas disponibles y retorna la seleccionada."""
...
def display_force_results(mode, target_idx, net_force, charges):
    """Imprime los resultados detallados del cálculo de fuerza neta."""
...
def save_force_plot(mode, charges, target_charge, net_force):
    """Genera y guarda el gráfico de fuerza, liberando la memoria."""
...
def ask_yes_no(question):
    """Solicita una respuesta s/n al usuario."""
    return input(f"\n{question} (s/n): ").strip().lower() == 's'


def calculate_and_display_field_at_point(mode, charges):
    """Solicita un punto al usuario, calcula el campo y muestra los resultados."""
    try:
...
def run_simulation(mode):
    """Ejecuta el flujo principal de simulación para un modo dado."""
